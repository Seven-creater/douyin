...
- 完整性校验（head_object 字节比对 + dataset 页面可见）：**已通过**，见下方结果栏；
- 未测路径：跨天续传（STS 12h 过期后 refresh 分支只在代码层面 review，未实际触发）；
  403 抖动重试路径未在 ststoken 获取处覆盖（首次 make_bucket 失败直接退出，需重跑脚本）。

## 结果

- 压缩版 3.63GB 上传完成：10:58:12 → 11:32:43，**34.5 分钟**（聚合 1.8 MB/s，38 片全程零重试零报错）；
- `complete_multipart_upload` 后 `list_oss_dataset_objects` 立即可见，远端 `Size=3,898,836,085`
  与本地逐字节相等（脚本内 head_object 比对 + 独立 API 列表双重确认）；
- 数据集页面「仓库文件」视图仍只显示几 KB 的 README——OSS 数据文件须看「数据文件」页，属正常分层，不是丢文件；
- 意外收获：`list_oss_dataset_objects` 直接返回带签名的 OSS 下载 URL（12h 有效），服务器侧
  取素材不需要走 msdatasets，拿 URL wget 即可。
...
5. 服务器侧拉取：`list_oss_dataset_objects` 返回的签名直链 12h 有效，aria2c 8 连接平均
   2.0 MiB/s（curl 单流 1.3 MiB/s，多连接有小幅收益）；**工具调用超时被移到后台时可能重跑整条命令**，
   造成两个 curl 带 `-C -` 交错写同一文件——必须杀光重下。且 `pkill -f 'curl -sSL'` 会匹配到
   自身 ssh 包装进程导致 255 假象，用 `pkill -x curl` 精确匹配。
6. 远端一条龙脚本传参：`ssh 'read -r U; ...'` 配 heredoc 会把 ssh 卡死在 read 上（实测
   wrapper 挂着、脚本没落地）——**本地写脚本 → scp → URL 走 argv** 才稳。罗小黑 29.4GB
   复用全流程：上传 4h53m（中途一次 -2 六片同断，重试 1/8 自愈）、服务器拉取+校验+解压
   自走完成，MD5 双端一致（66146571ab61c08cbb8fcff11cf8d0c5）。
